...
can then be rendered using Metanorma into a number of output formats
(https://www.metanorma.org/author/topics/building/output-formats/):
...
+- UML_EA.dtd
+- Images/
   +- EAID_40625194_4483_46b2_80CF_2756F08865D8.emf
   +- EAID_76FDCDFB_19E5_47b6_9D21_E6450814059F.emf
   +- EAID_9499129E_BD74_4df2_9AC5_680582E4CD47.emf
----
====

For typical UML diagrams, the "Metafile" format exports are in the Windows
Enhanced Metafile format (EMF, `*.emf`), and work best since they are vector
images. Metanorma automatically converts images from the EMF format into SVG,
which allows for perfect scaling in PDF output and in HTML web browsers.

However, the conversion from EMF to SVG can occasionally fail to produce
accurate results, especially for complex UML diagrams that involve custom
relationships and lines.

.Example of failed EMF to SVG conversion with original PNG
====
.Failed conversion of complex EMF image to SVG
image::ea-exported-image-svg.svg[]

.Original source of failed conversion in PNG
...
be placed alongside the previously generated EMF images as shown in
...
+
NOTE: The default style is used for <<OGC_20-040r3>>.
...
`EAID_40625194_4483_46b2_80CF_2756F08865D8.emf`, which are difficult to work
with.
...
[source,asciidoc]
----
[.diagram_include_block, base_path="working-directory/Images", format="emf"] <1>
....
Text <2>
....
----
...
.Example of OGC MDS document directory with EMF images
====
----
+- sources/
  +- images/
...
.Example to include EA-generated images in the `lutaml_uml_datamodel_description` block
====
[source,asciidoc]
----
[.diagram_include_block, base_path="model/Images", format="emf"]
....
....
----
====

EA also supports exporting PNG files with an XMI. When that is the case,
the `png` format option can be used.
...
Other image formats, such as SVG (using `format="svg"`), can also be used, as
long as the images are manually converted to SVGs such as by using the `emf2svg`
program or with EMF editors such as Adobe Illustrator.
